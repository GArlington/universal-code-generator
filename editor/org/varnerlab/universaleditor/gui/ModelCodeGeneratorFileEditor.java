...
public class ModelCodeGeneratorFileEditor extends javax.swing.JInternalFrame implements IVLTreeGUI,IVLSystemwideEventListener,TableModelListener {
	...
	public void setRootNode(Document doc) throws Exception
	{
		...
		// ok, so we need to configure the containerList -
	...
	private void initComponents() {
		...
		jButton6.setIcon(new javax.swing.ImageIcon("./images/Newspaper-Feed-12-Grey.png")); // NOI18N
		...
		jButton6.setRolloverIcon(new javax.swing.ImageIcon("./images/Newspaper-Feed-12.png")); // NOI18N
	...
	private void createNewTree(java.awt.event.ActionEvent evt) {
		...
		try {
			...
			// Ok, store the tree -
			_session.setProperty("MODEL_TEMPLATE_FILE_TREE", doc);
		...
		if (strUserName!=null && strRemotePath!=null)
		{
			...
			if (jTree1!=null)
			{
				...
				for (int kid_index=0;kid_index<NUMBER_OF_KIDS;kid_index++)
				{
					...
					{
						// Set the value on the user object 
						userObj.setProperty(keyName,strRemotePath);
						
						// Get the underlying xmlNode and set the data -
						Node xmlNode = (Node)userObj.getProperty("XML_TREE_NODE");
						xmlNode.setTextContent(strRemotePath);
					...
					else if (keyName.equalsIgnoreCase("DATE_CREATED"))
					{
						...
						// Get the underlying xmlNode and set the data -
						Node xmlNode = (Node)userObj.getProperty("XML_TREE_NODE");
						xmlNode.setTextContent(strObjDate);
					}
					else if (keyName.equalsIgnoreCase("VERSION"))
					{
						...
						if (!strVer.isEmpty())
						{
							...
							// Get the underlying xmlNode and set the data -
							Node xmlNode = (Node)userObj.getProperty("XML_TREE_NODE");
							xmlNode.setTextContent(String.valueOf(dblVer));
						}
	...
	public void tableChanged(TableModelEvent e) {
        ...
        if (ROW_INDEX!=-1)
        {
            ...
            if (key!=null || !tmp.equalsIgnoreCase(" "))
            {
                ...
                xmlTreeNode.setTextContent(tmp);
                
                /* We are going to assume that we are not storing *user data* in attributes
                // Set the data on the attributes -
