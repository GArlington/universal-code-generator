...
public class LoadVarnerFlatFileFBASiganling implements IInputHandler {
	...
	public LoadVarnerFlatFileFBASiganling()
	{
		// Load the prop table -
	...
	public Object getResource(Object object) throws Exception {
		...
		// Add _b species to model -
		addBoundarySpeciesToModel();
		
		// Add the parameters to the model - we are doing FBA don't need the parameters -
	...
	private void addBoundarySpeciesToModel() throws Exception
	{
		// Method attributes -
		int counter = 0;
		...
		while (species_iter.hasNext())
		{
			...
			// Get the model name -
			newSpecies.setCompartment("BOUNDARY");
	...
	private void addExchangeReactionsToModel() throws Exception
	{
		// Method attributes -
		int counter = 0;
		...
		while (species_iter.hasNext())
		{
			...
			// append a _b to the name -
			String spBName = spName+"_b";
			
			// Create the reaction -
			Reaction forward = buildExchangeReaction(spName,spBName,counter);
			
			// add the reaction to the model -
			_modelWrapper.addReaction(forward);
			
			// Create the reverse reaction -
			Reaction reverse = buildExchangeReaction(spBName,spName,counter+1);
			
			// add the reaction to the model -
			_modelWrapper.addReaction(reverse);
	...
	private Reaction buildExchangeReaction(String strReactant,String strProduct,int counter) throws Exception
	{
		// Create a new SBML reaction object -
		Reaction rxnSBMLExchange = _modelWrapper.createReaction();
		
		// flat files always are irreversible -
		rxnSBMLExchange.setReversible(false);
		
		// Create and configure species reference -
		SpeciesReference specRefForward = rxnSBMLExchange.createReactant();
		SpeciesReference specRefRev = rxnSBMLExchange.createProduct();
		specRefForward.setSpecies(strReactant);
		specRefForward.setStoichiometry(1.0);
		specRefRev.setSpecies(strProduct);
		specRefRev.setStoichiometry(1.0);
		
		// Add the reactants/products to this reaction -
		rxnSBMLExchange.addReactant(specRefForward);
		rxnSBMLExchange.addProduct(specRefRev);
		
		rxnSBMLExchange.setName("EXCHANGE_"+strReactant);
		rxnSBMLExchange.setId("EXCHANGE_R_"+counter);
		
		// Return to the caller -
		return (rxnSBMLExchange);
	}
	...
	private void addCompartmentToModel() throws Exception
	{
		...
		// OK, so then add the BOUNDARY compartment -
		Compartment tmpBCompartment = _modelWrapper.createCompartment();
		tmpBCompartment.setName("BOUNDARY");
		tmpBCompartment.setId("BOUNDARY");
		_modelWrapper.addCompartment(tmpBCompartment);
	}
